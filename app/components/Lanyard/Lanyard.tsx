...
export default function Lanyard({ position = [0, 0, 30], gravity = [0, -40, 0], fov = 20, transparent = true }: LanyardProps) {
  // Optimize for mobile - reduce FOV on small screens
  const isMobile = typeof window !== "undefined" && window.innerWidth < 768;
  const optimizedFov = isMobile ? 25 : fov;
  const optimizedPosition = isMobile ? ([0, 0, 35] as [number, number, number]) : position;
  ...
  return (
    <div className="relative z-0 w-full h-screen flex justify-center items-center transform scale-100 origin-center">
      <Canvas
        camera={{ position: optimizedPosition, fov: optimizedFov }}
        gl={
          {
            alpha: transparent,
            antialias: !isMobile,
        ...
        onCreated={({ gl }) => {
          ...
          // Handle context loss for the canvas
  ...
  return (
    <>
      ...
      <mesh ref={band}>
        ...
        <meshLineGeometry />
        {/* @ts-ignore */}
        <meshLineMaterial color="white" depthTest={false} resolution={isSmall ? [800, 1200] : [1200, 1000]} useMap map={texture} repeat={[-4, 1]} lineWidth={isSmall ? 0.8 : 1} />
      </mesh>
    </>
  );
}
