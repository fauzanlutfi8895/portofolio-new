...
const Squares: React.FC<SquaresProps> = ({ direction = "right", speed = 1, borderColor = "#999", squareSize = 40, hoverFillColor = "#222" }) => {
  ...
  useEffect(() => {
    ...
    const drawGrid = () => {
      ...
      // Optimize: reduce drawing on mobile
      const isMobile = isMobileScreen();
      const skipFactor = isMobile ? 1 : 1;

      for (let x = startX; x < canvas.width + squareSize; x += squareSize * skipFactor) {
        for (let y = startY; y < canvas.height + squareSize; y += squareSize * skipFactor) {
