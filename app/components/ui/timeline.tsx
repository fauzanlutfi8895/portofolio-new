        ...
        {data.map((item, index) => (
          <div key={index} className="flex justify-start pt-10 md:pt-40 md:gap-10">
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              ...
              <h3 className="hidden md:block text-xl md:text-5xl font-bold md:pl-20 text-neutral-500 dark:text-neutral-500 tracking-tight">{item.title}</h3>
            </div>
            ...
            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              ...
              {/* ✅ Content */}
              <div className="mb-6 text-neutral-700 dark:text-neutral-300">{item.content}</div>

              {/* ✅ Image */}
              {item.image && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-7 pb-4">
                  {item.image.map((src, i) => (
                    <div key={i} className="relative w-[300px] md:w-[420px] h-64 overflow-hidden rounded-xl border border-neutral-200 dark:border-neutral-800 shadow-sm hover:shadow-md transition-shadow">
                      <Image src={src} alt={`${item.title}-${i}`} fill className="object-cover hover:scale-105 transition-transform duration-300" />
                    </div>
                  ))}
                </div>
              )}
